...
# -*- coding: utf-8 -*-
"""
VGG16 — Multimodal training for Chinese Medicinal Herb classification.
Training strategy STRICTLY follows MNV2_text.py (MobileNetV2 multimodal baseline).

Architecture (identical to MNV2_text.py):
  Visual branch: VGG16 (224×224) → GAP → BN → Dense(512, swish)
...
⚠ VGG16 NOTE: VGG16 has NO BatchNormalization layers (unlike modern architectures).
  The BN-freezing code in the shared config handles this gracefully (the isinstance check
  simply never matches, so all layers in the unfrozen portion remain trainable). This is
  the correct behavior and matches the original MNV2_text.py intent.
"""
import os
import sys
...
def build_vgg16_backbone(input_tensor=None):
    """Build VGG16 backbone, optionally with a specific input_tensor."""
    kwargs = dict(include_top=False, weights='imagenet')
    if input_tensor is not None:
        kwargs['input_tensor'] = input_tensor
    else:
        kwargs['input_shape'] = (*IMAGE_SIZE, 3)
    return tf.keras.applications.VGG16(**kwargs)


# VGG16 native input size
IMAGE_SIZE = (224, 224)

# VGG16 preprocessing: RGB → BGR, zero-center each channel
PREPROCESS_FN = tf.keras.applications.vgg16.preprocess_input

OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "output_vgg16_multimodal")


if __name__ == "__main__":
    print("=" * 70)
    print("VGG16 MULTIMODAL Training")
    ...
    print(f"⚠ VGG16 has no BN layers — all unfrozen layers trainable (by design)")
